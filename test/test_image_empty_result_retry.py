...
class ImageEmptyResultRetryTests(unittest.TestCase):
    ...
    def test_reference_image_prompt_forces_edit_and_outpaint_intent(self) -> None:
        prompt = conversation.build_image_prompt("拓展画布到16：9的形式", "16:9", has_reference_images=True)
    ...
    def test_json_image_parameter_text_is_sanitized(self) -> None:
        message = (
            ...
            '"size":"1024x1024","referenced_image_ids":["file_123"]}'
        )

        self.assertTrue(conversation.is_image_parameter_text_response(message))
        self.assertIn("图片参数文本", conversation.image_stream_error_message(message))
        self.assertNotIn("referenced_image_ids", conversation.image_stream_error_message(message))

    def test_json_image_parameter_text_response_does_not_wait_for_polling(self) -> None:
        class FakeBackend:
            access_token = "token-1"

            def resolve_conversation_image_urls(self, *args, **kwargs):
                raise AssertionError("should not poll when upstream returned image params as text")
        ...
        with mock.patch.object(conversation, "conversation_events", return_value=iter(events)):
            with self.assertRaisesRegex(conversation.TextOnlyImageResponseError, "图片参数文本"):
                list(
    ...
    def test_tool_not_invoked_does_not_wait_for_image_poll_timeout(self) -> None:
        class FakeBackend:
            access_token = "token-1"

            def resolve_conversation_image_urls(self, *args, **kwargs):
                raise AssertionError("should not poll when upstream did not invoke the image tool")
        ...
        with mock.patch.object(conversation, "conversation_events", return_value=iter(events)):
            with self.assertRaisesRegex(conversation.TextOnlyImageResponseError, "未启动图片生成工具"):
                list(
                    conversation.stream_image_outputs(
                        FakeBackend(),
                        conversation.ConversationRequest(
